...
Chord validation verifies support for the intended symbol. It does not claim to enumerate every plausible contextual analysis. Generated isolated sonorities explicitly enumerate the straightforward sixth/minor-seventh and minor-sixth/half-diminished equivalents in `acceptableChordSymbols`; validation rejects a generated item if one of those readings is omitted. Spelled roots remain exact, so an enharmonic respelling is not accepted automatically. The visible `CID-XXXXXXX` token encodes the generator state: opening `?variant=CID-XXXXXXX` recreates the same four key signatures, voicings, basses and accepted analyses.
