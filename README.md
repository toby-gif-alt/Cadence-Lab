...
`question-bank.js` contains 55 hand-authored templates:

| Family | Count |
| --- | ---: |
| Roman-numeral analysis | 10 |
| Keys and modulation | 9 |
| SATB / vocal completion | 5 |
| Piano completion | 10 |
| Jazz / rock notation | 8 |
| Harmonic or tonal feature | 13 |

Authored questions are classified as `nzqa-reference`, `practice-assessment-reference` or `original-practice`; generated chord sets are separately labelled `generated-practice`. The bank currently retains twenty exact official references: 2021 Bach Q1(a), 2022 Bach Q1(a), 2023 Bach Q1(b), all official 2024 subquestions across Bach Q1, Rimsky-Korsakov Q2 and Phillip Norman Q3, plus all nine 2025 subquestions across Bach Q1, Schubert Q2 and Billy Joel Q3. The complete 2024 and 2025 examination papers are therefore represented as exact NZQA reference practice. No Learning Ideas item currently meets the exact-transcription contract, so those useful reductions are labelled adapted original practice and the practice-reference count is zero. Each retained reference records provider, year, question, part, extract, bars, source kind, source-page location and acknowledgement metadata. The PDFs are external source material and are not stored in the repository. [`docs/source-audit-2021-2025.md`](docs/source-audit-2021-2025.md) is the full question/schedule manifest; `docs/nzqa-task-map.md` retains the concise official-task overview.
...
The 55 source-based and original questions remain fully authored. A separate controlled generator creates four independent vertical chord-identification prompts from a verified semantic catalogue; it does not generate SATB, modulation or Bach-style passages and never alters a reference.
...
`measure` and `beat` carry the musical location; `event` is a zero-based anchor within that measure. `answerRole` is always explicit: `supplied` displays an authored question label, `editable` maps one-to-one to a semantic response slot, and `none` creates no browser-editable answer. Layout or proportional anchors never imply an interactive box by themselves. A paper-completion task may separately mark a harmonic position as a noninteractive paper choice box so the learner receives a blank printed box without creating browser answer state. A melody can therefore contain many note events while creating only one authored analysis position. Answer boxes are drawn inside the score SVG and the renderer calculates top and bottom decoration space per system from the actual overlays, including modulation brackets and pitch-specific note markers.
...
Piano completions may likewise use `staffVoices` and `questionStaffVoices` to keep melody, inner parts and accompaniment as independent rhythmic layers. The question data can therefore retain only the supplied melody while the separately rendered model adds a genuinely active, playable texture. Tie resolution follows the authored staff, voice role, source event and exact pitch rather than a shared harmonic anchor.
...
Explicit measures are assigned a deterministic preferred width from their rhythmic onsets, shortest value, independent-voice activity, accidentals and simultaneous accidental columns, rests, dots, ties, close-position seconds/unisons, analysis boxes and note annotations. Measures are packed only while their preferred widths fit the system; remaining width is distributed proportionally. VexFlow's own minimum formatting width is also recorded during formatting for QA. Dense chorale and chromatic chord bars therefore receive more horizontal space than sparse bars, and narrow screens reflow to additional systems instead of compressing every bar equally. SATB inter-staff distance also increases modestly for dense or collision-prone systems. Printed rhythm cues use pitchless Bravura duration glyphs rather than arbitrary VexFlow pitches or operating-system music-font characters. The shared renderer aligns each cue to a formatted musical onset when one exists, falls back to proportional measure timing for arbitrary subdivisions, and places it in a layout-aware lane: below the lower staff for SATB or single-staff work, between piano staves for grand-staff work, and between the piano staves beneath the vocal line for vocal-piano work. The cue lane follows its measure across responsive system breaks and is included in lower-analysis clearance.
...
- every harmonic event declares `supplied`, `editable` or `none`, every editable event maps to exactly one response slot, paper completions contain no editable analysis boxes, and layout anchors cannot create extra browser answers;
...
- each declared `9–8` or `4–3` suspension has a consonant preparation, visible tie/retention, the stated dissonant interval above the displayed bass, a contiguous downward-step resolution and matching harmonic span/prose;
- all SATB events contain four named voices in non-crossing order;
- original SATB models pass a hard part-writing audit, while a separate style-quality audit flags questionable doubling, avoidable common-tone/leap choices, repeated or excessive inner-part motion, repeated similar motion and needlessly wide texture for documented human review;
- original/adapted piano completions receive non-blocking QA warnings for simultaneous hand spans over an octave, hand overlap, abrupt register jumps, supplied-line loss or inner notes crossing the supplied melody;
...
Serve the repository root over HTTP, then open `tests/renderer-smoke.html`. The renderer smoke page validates the full bank and renders every question and model score at 1000, 780 and 390 pixels, including exact semantic box-count checks. Open `tests/interaction-smoke.html` for the whole-bank spoiler audit, all paper contracts, question-only versus model playback, A4 question/model print separation, contextual self-check contracts, pivot-only dual Roman controls, auto-advance, semantic key relationships, exhaustive authored-jazz builder construction, ambiguity coverage, visible-Variant-ID round trips and hundreds of seeded generated variants. `tests/exact-2025-q1-smoke.html`, `tests/exact-2025-q2-smoke.html` and `tests/exact-2025-q3-smoke.html` lock the three exact 2025 question groups, including Q3's vocal-plus-piano layout, source-supplied chord labels, full bridge, paper mask, rhythmic cues and published model; `tests/integrity-correction-smoke.html` exercises the same corrections through the real production script order. `tests/musical-validity-smoke.html` runs through the production page and covers suspension/NHT evidence, SATB hard and style audits, piano texture/playability, prompt secrecy, one-to-one answer boxes, dense accidental geometry, the exact Q1(c) D-to-D tie and native responsive rhythm cues. `tests/visual-gallery.html?source=nzqa-reference&year=2025&question=Question%20Three&width=1000` filters the exact Q3 references. Width accepts any value from 340 to 1040 pixels, and the gallery surfaces all manual-review warnings.
